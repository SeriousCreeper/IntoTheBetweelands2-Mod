...
@Mixin(value = ItemBoundAxe.class, remap = false)
public class MixinItemBoundAxe extends ItemBoundTool implements IActivatable, ICorrodible, IAnimatorRepairable {
    @Final
    @Shadow
    private static Set<Block> EFFECTIVE_ON;

    public MixinItemBoundAxe(String name, float damage, Set<Block> effectiveBlocks) {
    ...
    protected void onBoundRelease(ItemStack stack, World world, EntityPlayer player, int charge) {
        if (!world.isRemote) {
            ...
            for(int i = -range; i <= range; ++i) {
                for(int j = 0; j <= 2 * range; ++j) {
                    for(int k = -range; k <= range; ++k) {
                        ...
                        if (!world.isAirBlock(blockPos) && (blockState.getMaterial() == Material.WOOD || EFFECTIVE_ON.contains(blockState.getBlock()))) {
            ...
            world.createExplosion(player, (double)playerPos.getX(), (double)playerPos.getY(), (double)playerPos.getZ(), 0.1F, false);
        }
    }
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot equipmentSlot, ItemStack stack) {
        ...
        if (equipmentSlot == EntityEquipmentSlot.MAINHAND) {
            multimap.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(), new AttributeModifier(ATTACK_DAMAGE_MODIFIER, "Weapon modifier", CorrosionHelper.getModifier(stack) * (this.getActivated(stack) ? 7.0 : 2.0), 0));
